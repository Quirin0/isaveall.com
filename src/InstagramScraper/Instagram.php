...
/**
    ...
    */
    ...
    public function getAccountInfo($username)
    {
        ...
        if (static::HTTP_OK !== $response->code) {
            throw new InstagramException('Response code is here ' . $response->code . ': ' . static::httpCodeToString($response->code) . '.' .
                                         'Something went wrong. Please report issue.', $response->code, static::getErrorBody($response->body));
        }
        ...
        if (!isset($userArray['graphql']['user'])) {
            throw new InstagramException('Response code is here 2' . $response->code . ': ' . static::httpCodeToString($response->code) . '.' .
                                         'Something went wrong. Please report issue.', $response->code, static::getErrorBody($response->body));
        }
